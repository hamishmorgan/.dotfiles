...
# - Shells can be nested (e.g., bash inside tmux inside Zed)
...
# Known terminal emulators that are definitely not IDE integrated terminals
_is_standalone_terminal() {
  case "${TERM_PROGRAM:-}" in
    WezTerm|iTerm.app|Apple_Terminal|Alacritty|kitty|Hyper|Ghostty)
      return 0
      ;;
  esac
  return 1
...
_is_editor_in_context() {
  ...
  case "$editor" in
    zed)
      # Zed sets TERM_PROGRAM when running in its integrated terminal
      if [[ "${TERM_PROGRAM:-}" == "zed" ]]; then
        return 0
      fi
      _is_standalone_terminal && return 1
      pattern="zed"
      ;;
    code)
      ...
      ;;
    nvim|vim)
      # Skip process tree for nvim/vim in standalone terminals
      _is_standalone_terminal && return 1
      pattern="$editor"
      ;;
    *)
  ...
  # Fallback: check process tree (only for unknown terminal contexts)
  local editor_pid
